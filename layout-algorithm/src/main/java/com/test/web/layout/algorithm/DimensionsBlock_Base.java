...
abstract class DimensionsBlock_Base extends DimensionCase {
	...
	private static long computeHorizontalMarginsWhenHasWidthFromCSS(
  	...
  	static IWrapping initMarginsWhenKnownCSSWidth(IStyleDimensions margins, ContainerDimensions container, ILayoutStylesGetters styles, IWrapping padding, ElementLayout resultingLayout) {
  	...
  	static IWrapping initMarginsWhenNoCSSWidth(IStyleDimensions margins, ContainerDimensions container, ElementLayout resultingLayout) {
    ...
    static void initBoundsAndVerifyAutoMargins(IStyleDimensions layoutMargins, ContainerDimensions container, int containerLeft, int containerTop, int innerWidth, int innerHeight, IWrapping padding, IWrapping margins, ElementLayout resultingLayout) {
    	
    	initBounds(containerLeft, containerTop, innerWidth, innerHeight, padding, margins, resultingLayout);

    	// Verify margin and padding calculations
    	if (layoutMargins.getLeftType() == Justify.AUTO || layoutMargins.getRightType() == Justify.AUTO) {
    		if (resultingLayout.getOuter().getWidth() != container.getRemainingWidth()) {
